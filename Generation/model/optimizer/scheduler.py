...
def get_scheduler(optimizer: torch.optim.Optimizer, dataloader_length: int,
    ...
    if learning_rate is None:
        if args is None:
            ...
            learning_rate = args.laerning_rate
    if scheduler_type is None:
        if args is None:
            raise ValueError('Either scheduler_type or args nust be given.')
        else:
    ...
    if scheduler_type == 'StepLR':
    ...
    elif scheduler_type == 'CosienAnnealingLR':
        ...
        raise ValueError(f'Unknown scheduler optim {scheduler_type}')