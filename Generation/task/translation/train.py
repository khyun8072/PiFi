...
def training(args: argparse.Namespace) -> None:
    ...
    dataloader_dict['train'] = DataLoader(dataset_dict['train'], batch_size=args.batch_size, num_workers=args.num_workers, shuffle=True, pin_memory=True, drop_last=True)
    dataloader_dict['valid'] = DataLoader(dataset_dict['valid'], batch_size=args.batch_size, num_workers=args.num_workers, shuffle=False, pin_memory=True, drop_last=True)
    ...
    scheduler = get_scheduler(optimizer, len(dataloader_dict['train']), num_epochs=args.num_epochs, early_stopping_patience=args.early_stopping_patience, learning_rate=args.learning_rate, scheduler_type=args.scheduler)
    ...
    if args.job == 'resume_training':
        ...
        load_checkpoint_name = os.path.join(args.checkpoint_path, args.task, args.task_dataset, args.model_type, f'checkpoint.pt')
        model = model.to('cpu')
        ...
        if args.use_wandb:
            ...
            wandb.init(project=args.proj_name, name=get_wandb_exp_name(args), config=args, notes=args.description, tags=["TRAIN", f"Dataset: {args.task_dataset}", f"Model: {args.model_type}"], resume=True, id=checkpoint['wandb_id'])
    ...
    if args.use_wandb and args.job == 'training':
        import wandb
        from wandb import AlertLevel
        wandb.init(project=args.proj_name, name=get_wandb_exp_name(args), config=args, notes=args.description, tags=["TRAIN", f"Dataset: {args.task_dataset}", f"Model: {args.model_type}"])
        wandb.watch(models=model, criterion=cls_loss, log='all', log_freq=10)
    ...
    for epoch_idx in range(start_epoch, args.num_epochs):
        ...
        if best_valid_objective_value is None or valid_objective_value > best_valid_objective_value:
            ...
            checkpoint_save_path = os.path.join(args.checkpoint_path, args.task, args.task_dataset, args.model_type, args.method, args.llm_model, str(args.layer_num))
        ...
        if args.use_wandb:
            wandb.log({'TRAIN / Epoch_Loss': train_loss_cls / len(dataloader_dict['train']), 'VALID / Epoch_Loss': valid_loss_cls, 'Epoch_Index': epoch_idx})
            wandb.alert(title='Epoch End', text=f'VALID - Epoch {epoch_idx} - Loss: {valid_loss_cls:.4f}', level=AlertLevel.INFO, wait_duration=300)
    ...
    final_model_save_path = os.path.join(args.model_path, args.task, args.task_dataset, args.model_type, args.method, args.llm_model, str(args.layer_num))
    check_path(final_model_save_path)
    shutil.copyfile(os.path.join(checkpoint_save_path, 'checkpoint.pt'), os.path.join(final_model_save_path, 'final_model.pt'))
